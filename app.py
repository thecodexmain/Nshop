# app.py - Enhanced Shopify API with Multi-Site Support & Concurrency
import requests
import re
import json
import html
...
DEFAULT_SITES = [
    ...
    "https://mercihandy.com",
    "https://shop.mercihandy.com"
]

# Extended site pool with fallbacks
FALLBACK_SITES = [
    "https://us.mercihandy.com",
    "https://mercihandy.com",
    "https://shop.mercihandy.com",
    "https://checkout.mercihandy.com"
]
...
def get_session(proxy_url=None):
    ...
    session_key = proxy_url or "default"
    
    with _session_lock:
        if session_key not in _session_pool:
...
# ====================== ENHANCED CHECKOUT WITH MULTI-SITE SUPPORT ======================
def run_checkout(domain: str, card_number: str, card_month: str, card_year: str, 
                 card_cvv: str, proxy_config: Optional[dict] = None, under: int = 10,
                 sites: List[str] = None) -> dict:
    """Main checkout automation function with multi-site support and retries"""
    ...
    # Use provided sites or default
    if sites is None:
        sites = DEFAULT_SITES + FALLBACK_SITES
    ...
    for retry in range(max_retries):
        for site_idx, site in enumerate(sites):
            try:
                logger.info(f"Trying site: {site} (attempt {retry+1}/{max_retries})")
                
                # Setup session with proxy if provided
                session = get_session(proxy_config.get('http') if proxy_config else None)
                ...
                home_resp = session.get(site + '/', headers=home_headers, timeout=30)
                if home_resp.status_code != 200:
                    logger.warning(f"Home page returned {home_resp.status_code}, trying next site")
                    continue
                ...
                products_resp = session.get(site + '/products.json?limit=250', headers=products_headers, timeout=30)
                if products_resp.status_code != 200:
                    products_resp = session.get(site + '/products.json?limit=250&page=1', headers=products_headers, timeout=30)
                
                if products_resp.status_code != 200:
                    logger.warning(f"Products API returned {products_resp.status_code}, trying next site")
                    continue
                
                products = products_resp.json().get('products', [])
                
                if not products:
                    logger.warning("No products found, trying next site")
                    continue
                ...
                add_headers = {
                    ...
                    'origin': site,
                    'referer': f"{site}/products/{selected_product['handle']}?variant={selected_variant['id']}",
                    'sec-ch-ua': '"Chromium";v="142", "Google Chrome";v="142", "Not_A Brand";v="99"',
                    'sec-ch-ua-mobile': '?0',
                    'sec-ch-ua-platform': '"Windows"',
                    'sec-fetch-dest': 'empty',
                ...
                add_resp = session.post(site + '/cart/add', headers=add_headers, data=add_data, timeout=30)
                if add_resp.status_code != 200:
                    logger.warning(f"Add to cart failed with {add_resp.status_code}, trying next site")
                    continue
                ...
                cart_resp = session.get(site + '/cart.js', headers=cart_headers, timeout=30)
                if cart_resp.status_code != 200:
                    logger.warning(f"Cart API returned {cart_resp.status_code}, trying next site")
                    continue
                ...
                checkout_url = f"{site}/cart/c/{clean_token}"
                checkout_resp = session.get(checkout_url, params=checkout_params, headers=checkout_headers, 
                ...
                actions_url = extract_actions_js_url(html_text, site)
                processing_url = extract_processing_js_url(html_text, site)
                events_js_url = extract_events_js_url(html_text, site)
                ...
                if actions_url:
                    try:
                        js_body = fetch_js(session, actions_url, site, checkout_url_final)
                ...
                for source_js in [events_js_url, processing_url]:
                    if source_js and not poll_id:
                        try:
                            js = fetch_js(session, source_js, site, checkout_url_final)
                ...
                if not all([checkout_token, session_token, stable_id, build_id, proposal_id, submit_id]):
                    logger.warning("Failed to extract required tokens, trying next site")
                    continue
                ...
                proposal_resp = session.post(
                    f"{site}/checkouts/internal/graphql/persisted",
                ...
                if proposal_resp.status_code != 200:
                    logger.warning(f"Proposal 1 returned {proposal_resp.status_code}, trying next site")
                    continue
                ...
                addr = {
                    "first_name": random.choice(["Python", "John", "James", "Michael", "Robert"]),
                    "last_name": random.choice(["Shelby", "Smith", "Johnson", "Williams", "Brown"]),
                    "address1": f"{random.randint(100, 999)} {random.choice(['St', 'Ave', 'Blvd', 'Dr', 'Ln'])}",
                    "address2": "",
                    "city": random.choice(["New York", "Los Angeles", "Chicago", "Houston", "Phoenix", "Philadelphia", "San Antonio", "San Diego"]),
                    "country_code": "US",
                    "zone_code": random.choice(["NY", "CA", "IL", "TX", "AZ", "PA", "TX", "CA"]),
                ...
                if proposal2_resp.status_code != 200:
                    logger.warning(f"Proposal 2 returned {proposal2_resp.status_code}, trying next site")
                    continue
                ...
                if proposal3_resp.status_code != 200:
                    logger.warning(f"Proposal 3 returned {proposal3_resp.status_code}, trying next site")
                    continue
                ...
                shop_domain = site.replace("https://", "").replace("http://", "").split("/")[0]
                ...
                pci_session = get_session(proxy_config.get('http') if proxy_config else None)
                pci_resp = pci_session.post('https://checkout.pci.shopifyinc.com/sessions', 
                                           headers=pci_headers, json=pci_json, timeout=30)
                
                if pci_resp.status_code != 200:
                    logger.warning(f"PCI session returned {pci_resp.status_code}, trying next site")
                    continue
                ...
                logger.warning(f"Timeout for site {site}, trying next")
                continue
            except Exception as e:
                logger.error(f"Error with site {site}: {str(e)}")
                continue
        
        # If we've tried all sites, wait a bit before retrying
        if retry < max_retries - 1:
            time.sleep(1)
    
    # All attempts failed
    result['Code'] = 'ERROR'
    result['Response'] = 'All sites failed'
    elapsed = time.time() - start_time
    result['Time'] = f"{elapsed:.1f}s"
    return result
...
def check():
    """Main endpoint for Shopify checkout with multi-site support"""
    ...
    # Log request
    logger.info(f"Processing checkout: card={card_number[:4]}****, proxy={bool(proxy_config)}, sites={len(sites) if sites else len(DEFAULT_SITES)}")
...
@app.route('/shopify/v1/sites', methods=['GET'])
def get_sites():
    """Get list of available sites"""
    return jsonify({
        'default_sites': DEFAULT_SITES,
        'fallback_sites': FALLBACK_SITES,
        'all_sites': list(set(DEFAULT_SITES + FALLBACK_SITES))
    })
...
def index():
    return jsonify({
        'service': 'Shopify Checkout API (Enhanced)',
        'version': '2.0',
        'features': [
            'Multi-site support with automatic fallback',
        ...
        'endpoints': {
            ...
            '/shopify/v1/sites': 'GET available sites',
            '/health': 'Health check'
        },
        'example': '/shopify/v1/check?cc=4111111111111111|09|2027|123&proxy=ip:port:user:pass&under=10&sites=site1.com,site2.com'
    })
